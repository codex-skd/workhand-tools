...
- Fases 1-4 y 6 implementadas (delegado en OpenCode, verificado manualmente): `ModToolMaterials` con los 4 tiers propios, los 80 items (`ModItems`) con modelos placeholder y creative tab, las 81 recetas (72 shaped + robust_stick + 8 smithing de netherite), y los tags de encantamiento (`#minecraft:pickaxes`/`#minecraft:shovels` + `enchantable/durability|mining|mining_loot|vanishing`). Falta la Fase 5 (mecánica de minado en área).
