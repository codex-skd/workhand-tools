...
// Measurement box model: bounding-box math, Gizmo wireframe rendering, and length-label placement.
...
public class MeasurementBox {
    ...
    public void render(ResourceKey<Level> currentDimensionKey, PoseStack poseStack, SubmitNodeCollector nodeCollector, Camera camera, Matrix4fc projection) {
        ...
        float lineWidth = TapeMeasureConfig.CLIENT.lineWidth.get().floatValue();
        if (distance > 48) {
            lineWidth = TapeMeasureConfig.CLIENT.lineWidthMax.get();
        }
