...
// Measurement box model: bounding-box math, line rendering, and length-label placement.
// Ported essentially as-is from Mrbysco's "Measurements" mod (MIT). The original class credits
// MadeBaruna's BlockMeter project for the box-snap algorithm:
// https://github.com/MadeBaruna/BlockMeter/blob/master/src/main/java/win/baruna/blockmeter/MeasureBox.java
public class MeasurementBox {
    ...
    MeasurementBox(BlockPos block, ResourceKey<Level> dimensionKey) {
        ...
        this.lineColor = TapeMeasureConfig.CLIENT.lineColor.get().getColor(BoxHandler.random);
        TextColor textColor = TapeMeasureConfig.CLIENT.textColor.get();
        if (textColor == TextColor.XYZRGB) {
            this.textX = textColor.getColor(BoxHandler.random, Direction.Axis.X);
            this.textY = textColor.getColor(BoxHandler.random, Direction.Axis.Y);
            this.textZ = textColor.getColor(BoxHandler.random, Direction.Axis.Z);
        } else {
    ...
    public void render(ResourceKey<Level> currentDimensionKey, PoseStack poseStack, MultiBufferSource.BufferSource buffers, Camera camera, Matrix4f projection) {
        ...
        // 1.21.1 line rendering has a fixed width; lineWidth config is kept for TOML compatibility
        // but has no effect in this version.

        // RenderLevelStageEvent leaves the PoseStack in world space; the box AABB is in absolute
        // world coordinates, so translate by -cameraPos before drawing it (drawLength does its own
        // translate for the labels).
        poseStack.pushPose();
        poseStack.translate(-pos.x, -pos.y, -pos.z);
        LevelRenderer.renderLineBox(poseStack, buffers.getBuffer(RenderType.lines()), box, r, g, b, a);
        poseStack.popPose();

        drawLength(poseStack, camera, projection, buffers);
    }

    private void drawLength(PoseStack poseStack, Camera camera, Matrix4f projection, MultiBufferSource.BufferSource buffers) {
        ...
        final Frustum clippingHelper = new Frustum(projection, poseStack.last().pose());
        ...
        drawText(poseStack, camera, new Vec3(lineXPoint.x, lineXPoint.y, lineXPoint.z), Component.literal(String.valueOf(lengthX)), this.textX, buffers);
        drawText(poseStack, camera, new Vec3(lineYPoint.x, lineYPoint.y, lineYPoint.z), Component.literal(String.valueOf(lengthY)), this.textY, buffers);
        drawText(poseStack, camera, new Vec3(lineZPoint.x, lineZPoint.y, lineZPoint.z), Component.literal(String.valueOf(lengthZ)), this.textZ, buffers);
        poseStack.popPose();
    }
    ...
    private void drawText(PoseStack poseStack, Camera camera, Vec3 pos, Component length, DyeColor textColor, MultiBufferSource.BufferSource buffers) {
        final Font font = Minecraft.getInstance().font;
        final float size = TapeMeasureConfig.CLIENT.textSize.get().floatValue();
